...
export default {
  ...
  setup(editor) {
    ...
    if (isSamePath && isBothValid && isSameType && enableNotif)
      import_position.notify({ invalidFileTrue: false, isSameValid: true, notSupportedValid: 0 }, fromFileType, toFileType);
    else if (!isSamePath && !isBothValid && isSameType && enableNotif)
      import_position.notify({ invalidFileTrue: false, isSameValid: 0, notSupportedValid: true }, fromFileType, toFileType);
    else if (isSamePath && !isBothValid && isSameType && enableNotif)
      import_position.notify({ invalidFileTrue: false, isSameValid: false, notSupportedValid: 0 }, fromFileType, toFileType);
    else if (!isSamePath && !isBothValid && !isSameType && enableNotif)
      import_position.notify({ invalidFileTrue: false, isSameValid: 0, notSupportedValid: false }, fromFileType, toFileType);
    else if (!isSamePath && isBothValid && !isSameType && enableNotif)
      import_position.notify({ invalidFileTrue: true, isSameValid: 0, notSupportedValid: 0 }, fromFileType, toFileType);
    else if (!isSamePath && isBothValid && isSameType)
      this.importDragFile(relativePath, editor)
  },
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors((editor) => {
      ...
      this.subscriptions.add(lines, "drop", (_lines) => {
        ...
        leftDock.ondragend = (_leftDock) => {
          // const valid = _leftDock.target.className.includes('status');
          // valid ? this.setup(editor) : 0;
          this.setup(editor);
        }
      });
