...
export default {
  ...
  importDragFile(relativePath, editor) {
    ...
    this.param.importCursor
      ? import_position.importToSelectedLine(this.param, extname, importText, editor, importName, cursorPosition)
      : this.param.importPosition
        ? import_position.importToBottom(this.param, extname, importText, editor, importName, cursorPosition)
        : import_position.importToTop(this.param, extname, importText, editor, importName);
  },
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors((editor) => {
      ...
      this.subscriptions.add(lines, "drop", (_lines) => {
        ...
        leftDock.ondragend = (item) => {
          ...
          if (!activePane) return import_position.notify({ }, '', '');

          valid && activePane ? this.setup(item, activePane) : 0;
        };
      });
